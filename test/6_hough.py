...
new_lines = cv2.HoughLinesP(edge_image,1,np.pi/180,150,minLineLength=30,maxLineGap=200)
...
if new_lines is not None:
    for line in new_lines:
        x1, y1, x2, y2 = line[0]
        cv2.line(new_line_image,(x1,y1),(x2,y2),(0,255,0),1)

    new_combined_image = cv2.addWeighted(image,0.8,new_line_image, 1, 1)
    cv2.imshow('New_combined_image',new_combined_image)
    cv2.waitKey(0) 
